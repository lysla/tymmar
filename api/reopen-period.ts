...
export default async function handler(req: VercelRequest, res: VercelResponse) {
    try {
        ...
        if (typeof body.weekKey === "string" && /^(\d{4})-W(\d{2})$/.test(body.weekKey)) {
            ...
            // If the period doesn't exist and only weekKey is provided, we need weekStart to create it.
        ...
        await db.transaction(async (tx) => {
            ...
            if (!p) {
                // create an open period if missing
            ...
            // recompute total from hours (for consistency)
            const rows = await tx
